...
def train_one_epoch(epoch, total_steps, dataloader, model,
    ...
    for inputs in dataloader:
        ...
        up_labels = inputs['mask'].to(device)
        down_labels = F.interpolate(up_labels, mode='nearest', scale_factor=0.5)

        # forward pass
        up_pred, down_pred = model(images)

        # compute loss
        up_loss = criterion(up_pred, up_labels)
        down_loss = criterion(down_pred, down_labels)
        ...
        if show:
            ...
            raw_mask = up_labels[0, 0, :, :].data.cpu().numpy() * 255
            raw_mask = raw_mask.astype(np.uint8)
            prd_score = up_pred[0, 0, :, :].data.cpu().numpy()
            prd_mask = prd_score > 0.5


            viz.heatmap(images, win="train_image", opts={"title": "Train Image", "colormap": "Viridis"})
            viz.heatmap(prd_score, win="first prd score", opts={"title": "first pred score", "colormap": "Viridis"})
...
def eval_one_epoch(epoch, dataloader, model, device, log_file):
    with torch.no_grad():
        ...
        for inputs in dataloader:
            ...
            up_outputs, down_outputs = model(images)


            preds = up_outputs > threshold
...
def train_eval_model(opts):
    ...
    shutil.copy("./models/pe_net.py", os.path.join(model_dir, "pe_net.py"))
    shutil.copy("./trainer_pe_net.py", os.path.join(model_dir, "trainer_pe_net.py"))
...
if __name__ == '__main__':
    ...
    opts["log_dir"] = "./train_log/er_train_unet_aug_v1_20200705_iouloss"
